 ...
 * @flavars
/*********************************************************************************************************************************************************/
const uint16_t toneTable[89] PROGMEM = {
...
const uint8_t randomLookup[80] = {1, 1, 2, 1, 3, 4, 3, 1, 2, 2, 1, 1, 4, 4, 2, 2, 1, 1, 2, 2, 4, 3, 4, 4, 3, 1, 4,
                                  4, 4, 1, 2, 3, 2, 4, 3, 4, 2, 4, 2, 3, 3, 4, 1, 3, 1, 2, 4, 3, 1, 2, 3, 4, 3, 4,
                                  2, 3, 1, 2, 1, 4, 4, 3, 1, 2, 2, 3, 3, 1, 2, 4, 2, 3, 1, 4, 1, 3, 1, 2, 3, 4};
...
uint16_t Cubetto::getAnalog(uint8_t analog)
{
    ...
    {
        case 1:
            return analogRead(IO_ANA_1) * (3.2 * 2);
        case 2:
            return analogRead(IO_ANA_2) * (3.2 * 2);
        case 3:
            return analogRead(IO_ANA_3) * (3.2 * 2);
        default:
            return 0;
    }
}
...
void Cubetto::updateBatteryLevel()
{
    ...
    {
        batteryLevel = 100 - ((4000 - val) * 40) / 300;
    }
    else if (val > 3600)
    {
        batteryLevel = 42 - ((3700 - val) * 40) / 100;
    }
    else if (val > 3300)
    {
        batteryLevel = 18 - ((3600 - val) * 14) / 300;
    }
    else if (val > 3100)
    {
        batteryLevel = 6 - ((3300 - val) * 6) / 200;
    }
    else
    {
        batteryLevel = 0;
 ...
 * @brief          Decodes the bluetooth message instructions from Primo Interface and performs the actions requested. Call
if message is received on Bluetooth
    ...
    switch (command)
    {
        ...
        case CUBETTO_MESSAGE_MOVEMENT:
        {
            ...
            if (Direction & 0x80)
            {
                playForwardTone();
                Direction &= 0x7F;
            } // Check if we are the instruction of a sequence
            // Move direction
            switch (Direction)
            {
...
{
    left.move(STEPPER_TURN_STEPS_RIGHT);
    right.move(STEPPER_TURN_STEPS_RIGHT);
    move();
    playRightTone();
}
...
{
    left.move(-STEPPER_TURN_STEPS_LEFT);
    right.move(-STEPPER_TURN_STEPS_LEFT);
    move();
    playLeftTone();
}
 ...
 * @brief          Handles bluetooth housekeeping tasks, and checks for incoming messages. Call this at least once in the
loop ().
 * @param            None
 * @return        None
/*********************************************************************************************************************************************************/
 ...
 * @param            uint8_t timeout                 Time that Cubetto should advertise in ms. 0 causes cubetto to
constantly advertise.
 * @param            bool autoReconnect            Setting this causes Cubetto to automatically readvertise on Bluetooth
after it is disconnected from a Central device
 ...
 * @brief          Reply to Central device after receiving a message. Reply with TRUE (Received OK) or FALSE (Received
ERROR)
...
{
    if (note > sizeof(toneTable - 1))
    {
        return toneTable[sizeof(toneTable - 1)];
    }
    ...
    if (lib_aci_event_get(&aci_state, &aci_data))
    {
        ...
        switch (aci_evt->evt_opcode)
        {
            ...
            case ACI_EVT_DATA_RECEIVED:
                nrfOnWrite(aci_evt->params.data_received.rx_data.pipe_number,
